...
_TRAFFIC_COLOR: tuple[int, int, int] = (30, 180, 50)  # green
...
def make_traffic_light_image(image_size: int) -> tuple[Image.Image, dict[str, float | int | str]]:
    ...
    for _ in range(count):
        for _attempt in range(30):
            ...
            if any(abs(x - px) < box_size * 2 and abs(y - py) < box_size * 2 for px, py in used):
                continue
            used.append((x, y))
            draw.rectangle([x, y, x + box_size, y + box_size], fill=_TRAFFIC_COLOR, outline=(0, 0, 0), width=1)
            break
...
def write_split(
    ...
    with tl_meta.open("w", newline="", encoding="utf-8") as csv_file:
        writer = csv.DictWriter(
            csv_file,
            fieldnames=["image", "class_name", "traffic_lights"],
        )
        writer.writeheader()
        for idx in range(image_count):
            image_name = f"{idx:04d}.png"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--out-dir", type=Path, default=Path("data/tracks"))
...
def main() -> None:
    ...
    print("Classes: track, traffic_light")
