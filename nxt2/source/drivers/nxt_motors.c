...
void nxt_motor_quad_decode(nxt_motor_port *m, uint32 value)
{
    uint32 dir = value & 2;
    uint32 edge = value & 1;
...
void nxt_motor_isr_handler(void)
{
    ...
    /* Motor A */
    pins = ((current_pins >> MA0) & 1) | 
        ((current_pins >> (MA1 - 1)) & 2);
    nxt_motor_quad_decode(&motor_ports[0], pins);

    /* Motor B */
    pins = ((current_pins >> MB0) & 1) | 
        ((current_pins >> (MB1 - 1)) & 2);
    nxt_motor_quad_decode(&motor_ports[1], pins);

    /* Motor C */
    pins = ((current_pins >> MC0) & 1) | 
        ((current_pins >> (MC1 - 1)) & 2);
