...
void bt_init(void)
{
    ...
    *AT91C_PIOA_OER = BT_ARM7_CMD_PIN;
    ...
    /* Configure timer 1 as trigger for ADC, sample every 0.5ms. */
...
uint32 bt_pending()
{
    ...
    if (bytes_ready > in_buf_idx)
        ret |= 1;
    if ((*AT91C_US1_TCR != 0) || (*AT91C_US1_TNCR != 0))
        ret |= 2;
    ...
    if (total_bytes_ready > in_buf_idx + 1)
    {
        ...
        if (bytes_ready >= in_buf_idx + cmd_len + 1)
        {
            for (i = 0; i < cmd_len + 1; i++)
                buf[i] = buf_ptr[in_buf_idx++];
        }
        else
        {
            if (total_bytes_ready >= in_buf_idx + cmd_len + 1)
            {
                for (i = 0; i < cmd_len + 1 && in_buf_idx < 128; i++)
                    buf[i] = buf_ptr[in_buf_idx++];
                in_buf_idx = 0;
                tmp_ptr = &(in_buf[(in_buf_in_ptr + 1) % 2][0]);
                for (; i < cmd_len + 1; i++)
                    buf[i] = tmp_ptr[in_buf_idx++];
                in_buf_idx += 128;
            }
            else
...
uint32 bt_read(uint8 *buf, uint32 off, uint32 len)
{
    ...
    if (total_bytes_ready > in_buf_idx)
    {
        ...
        {
            for (i = 0; i < cmd_len && in_buf_idx < 128; i++)
            {
                buf[off + i] = buf_ptr[in_buf_idx++];
            }
            in_buf_idx = 0;
...
void bt_reset(void)
{
     ...
     * delays in it and so should probably only be called during the
     * application initialisation and termination. Calling it at other
     * times may cause problems for other real time tasks. */

    /* If power is currently off to the BC4 do not reset it! */
    if ((*AT91C_PIOA_ODSR & BT_RST_PIN) == 0)
        return;
